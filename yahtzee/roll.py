...
class Roll:
    ...
    def keep_dice(self, player_name_caps):
        ...
        keep_all = get_keep_dice_check(
            f"{player_name_caps} do you want to " f"KEEP ALL dice?\n"
        )
        if keep_all == "no":

            # ask if the user wants to REROLL ALL the dice
            reroll_all = get_keep_dice_check(f"Do you want to REROLL ALL " f"dice?\n")
            if reroll_all == "no":
                ...
                while True:
                    ...
                    if keep_some == "":
                        ...
                        keep_none_check = get_keep_dice_check(
                            f"Are you sure you want to REROLL " f"ALL the dice?\n"
                        )
    ...
    def reroll_dice(self, dice_list):
        ...
        self.current_dice_list = [randint(1, 6) for d in range(0, (len(dice_list)))]
    ...
    def check_full_house(self, dice_list):
        ...
        if (
            len(set(dice_list)) == 2
            and len([d for d in dice_list if dice_list.count(d) == 3]) == 3
        ):
            return 25
