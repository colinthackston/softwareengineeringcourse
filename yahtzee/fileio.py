...
class FileWriter:
    ...
    def write_file(
        self, datetime_today, game_counter, players_list, ranking_dict, file_formats
    ):
        """
        ...
        for file_format in file_formats:
            if file_format == "txt":
                ...
                # write textfile
                textfile.write_textfile(game_counter, players_list, ranking_dict)
                ...
                # write docxfile
                docxfile.write_docxfile(game_counter, players_list, ranking_dict)
    ...
    def write_textfile(self, game_counter, players_list, ranking_dict):
        ...
        with open(f"{self.textfile_dir_str}/{self.textfile_name}", "w") as f:
            ...
            for j, player in enumerate(players_list):
                f.write(f"\n{'-'*21}")
                f.write(f"\n{'-'*21}\n")
                f.write(f"{players_list[j].name.upper()} " f"FINAL SCORES".center(21))
            ...
            print_fileio_confirmation(self.textfile_dir_str, self.textfile_name)
...
class DocxFile:
    ...
    def write_docxfile(self, game_counter, players_list, ranking_dict):
        ...
        doc.add_picture(str(Path.cwd() / "yahtzee/resources/yahtzeePicture.jpg"))
        ...
        for j, player in enumerate(players_list):
            ...
            doc.add_paragraph(
                f"Top Bonus Score: " f"{players_list[j].get_top_bonus_score()}"
            )

            # write total scores and grand total
            doc.add_heading("TOTAL SCORES", 3)
            doc.add_paragraph(f"Total Top: " f"{players_list[j].get_total_top_score()}")
            doc.add_paragraph(
                f"Total Bottom: " f"{players_list[j].get_total_bottom_score()}"
            )
            para_obj_grand_total = doc.add_paragraph(
                f"GRAND TOTAL: " f"{players_list[j].get_grand_total_score()}"
            )

            # add pagebreak before writing next player scores to docx
            if j != (len(players_list) - 1):
                para_obj_grand_total.runs[0].add_break(docx.enum.text.WD_BREAK.PAGE)
